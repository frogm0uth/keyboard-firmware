...
void custom_mouse_action(void) {
    ...
    uint8_t        accel         = IS_MOUSE_SLOW ? 3 - delta : 4 + delta;
...
void custom_wheel_action(void) {
    ...
    wheel_repeat_term = IS_MOUSE_SLOW ? WHEEL_REPEAT_TERM * 2 : (IS_MOUSE_ACC2 ? WHEEL_REPEAT_TERM / 2 : WHEEL_REPEAT_TERM);
...
void custom_mouse_encoder(bool clockwise) {
    uint16_t keycode = (IS_MOUSE_REPT) ? (clockwise ? CM_MS_D : CM_MS_U) : (clockwise ? CM_MS_R : CM_MS_L);
...
void custom_wheel_encoder(bool clockwise, bool rotate) {
    uint16_t keycode = rotate ? (clockwise ? CM_MS_D : CM_MS_U) : (clockwise ? CM_MS_R : CM_MS_L);
