...
// Macro to access shortcuts. Use this in your code, not anything further down.
// The argument must be a SC_... keycode. NB: the argument must be a constant,
// you can't pass a variable to a macro.
//
...
#else
  // selected at runtime
  //#define SC(k) (os_shortcut_codes[OS_SC_OFFSET(k)][os_index])
...
// Custom keycodes for OS selection, Must be put in the
// custom_keycodes enum if using runtime shortcode selection.
//
#define OS_SELECT_KEYCODES    \
  CU_SELECT_MACOS,	      \
  CU_SELECT_WINDOWS

// Macro to define custom keycodes for all shortcuts. Must be included
// in the custom_keycodes enum. The first item MUST be SC_ID_START. If
// you add to this list, also add the definitions to
// static_shortcuts.h and add OS_SC_DEFINE lines in shortcuts.c.
//
